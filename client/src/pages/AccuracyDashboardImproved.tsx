...
export function AccuracyDashboardImproved() {
  ...
  const { data: cpuData, error: cpuError, loading: cpuLoading } = useAccuracyData('V135-POS-03', 'CPU', selectedHorizon, 30000, selectedBucket);
  const { data: memoryData, error: memoryError, loading: memoryLoading } = useAccuracyData('V135-POS-03', 'Memory', selectedHorizon, 30000, selectedBucket);
  const currentData = selectedMetric === 'CPU' ? cpuData : memoryData;
  const currentError = selectedMetric === 'CPU' ? cpuError : memoryError;
  const isLoading = selectedMetric === 'CPU' ? cpuLoading : memoryLoading;

  // CPU 또는 Memory 중 하나라도 데이터가 있으면 UI 표시
  const hasAnyData = cpuData?.records?.length > 0 || memoryData?.records?.length > 0;
  ...
  const errorChartOption = useMemo(() => {
    if (!currentData?.records?.length) {
      return {
        ...
        xAxis: { type: 'category', data: [], boundaryGap: false, axisLabel: { color: '#94a3b8', fontSize: 10 }, axisLine: { lineStyle: { color: '#334155' } } },
        yAxis: { type: 'value', axisLabel: { formatter: '{value}%', color: '#94a3b8', fontSize: 10 }, axisLine: { lineStyle: { color: '#334155' } }, splitLine: { lineStyle: { color: '#1e293b' } } },
    ...
    return {
      ...
      grid: { left: '8%', right: '5%', top: '12%', bottom: '12%' },
      xAxis: { type: 'category', data: timestamps, boundaryGap: false, axisLabel: { color: '#94a3b8', fontSize: 10 }, axisLine: { lineStyle: { color: '#334155' } } },
      yAxis: { type: 'value', max: yAxisMax, axisLabel: { formatter: '{value}%', color: '#94a3b8', fontSize: 10 }, axisLine: { lineStyle: { color: '#334155' } }, splitLine: { lineStyle: { color: '#1e293b' } } },
      series: [
        {
          name: '정상 오차',
          data: normalData,
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0f172a', color: '#e2e8f0', display: 'flex', flexDirection: 'column', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      ...
      <header style={{
        ...
        <h1 style={{ margin: '0 0 6px 0', fontSize: '28px', fontWeight: '700', letterSpacing: '-0.5px' }}>예측 신뢰도 & 정확성</h1>
        <p style={{ margin: 0, fontSize: '13px', color: '#94a3b8', fontWeight: '400' }}>우리의 예측을 얼마나 신뢰할 수 있는가?</p>
      </header>

      <main style={{ flex: 1, padding: '28px 32px', overflow: 'auto' }}>
        {/* Loading/Error State */}
        {isLoading && (
          <div style={{ backgroundColor: '#1e293b', borderRadius: '8px', border: '1px solid #334155', padding: '20px', marginBottom: '24px', textAlign: 'center', color: '#94a3b8' }}>
            Loading accuracy data...
          </div>
        )}
        {currentError && (
          <div style={{ backgroundColor: '#1e293b', borderRadius: '8px', border: '1px solid #ef4444', borderLeft: '3px solid #ef4444', padding: '20px', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '24px' }}>
          <div style={{ backgroundColor: '#1e293b', borderRadius: '8px', border: '1px solid #334155', padding: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              {['CPU', 'Memory'].map(m => (
                <button key={m} onClick={() => setSelectedMetric(m)} style={{ padding: '6px 14px', borderRadius: '6px', border: selectedMetric === m ? '1px solid #3b82f6' : '1px solid #334155', backgroundColor: selectedMetric === m ? '#3b82f615' : 'transparent', color: selectedMetric === m ? '#3b82f6' : '#94a3b8', cursor: 'pointer', fontSize: '11px', fontWeight: selectedMetric === m ? '600' : '400', transition: 'all 0.2s' }}>
                  {m}
                </button>
              ))}
            </div>
          ...
          <div style={{ backgroundColor: '#1e293b', borderRadius: '8px', border: '1px solid #334155', padding: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              {[
                ...
                <button key={b.value} onClick={() => setSelectedBucket(b.value)} style={{ padding: '6px 14px', borderRadius: '6px', border: selectedBucket === b.value ? '1px solid #10b981' : '1px solid #334155', backgroundColor: selectedBucket === b.value ? '#10b98115' : 'transparent', color: selectedBucket === b.value ? '#10b981' : '#94a3b8', cursor: 'pointer', fontSize: '11px', fontWeight: selectedBucket === b.value ? '600' : '400', transition: 'all 0.2s' }}>
                  {b.label}
                </button>
              ))}
            </div>
          ...
          <div style={{ backgroundColor: '#1e293b', borderRadius: '8px', border: '1px solid #334155', padding: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {[
                ...
                <button key={h.value} onClick={() => setSelectedHorizon(h.value)} style={{ padding: '6px 14px', borderRadius: '6px', border: selectedHorizon === h.value ? '1px solid #3b82f6' : '1px solid #334155', backgroundColor: selectedHorizon === h.value ? '#3b82f615' : 'transparent', color: selectedHorizon === h.value ? '#3b82f6' : '#94a3b8', cursor: 'pointer', fontSize: '11px', fontWeight: selectedHorizon === h.value ? '600' : '400', transition: 'all 0.2s' }}>
                  {h.label}
                </button>
              ))}
            </div>
        ...
        <div style={{ marginBottom: '24px' }}>
          <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', fontWeight: '700', color: '#e2e8f0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>의사결정 기준</h3>
        ...
        <div style={{ backgroundColor: '#1e293b', borderRadius: '8px', border: '1px solid #334155', padding: '20px', overflow: 'auto' }}>
          <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', fontWeight: '700', color: '#e2e8f0' }}>최근 오차율</h3>
